...
class _DenseLayer(nn.Module):
    def __init__(self, num_input_features, growth_rate, bn_size, drop_rate, quant, efficient=False):
    ...
    def forward(self, *prev_features):
        bn_function = _bn_function_factory(self.norm1, self.relu1, self.conv1, self.quant)
        if self.efficient and any(prev_feature.requires_grad for prev_feature in prev_features):
            bottleneck_output = cp.checkpoint(bn_function, *prev_features)
        else:
            bottleneck_output = bn_function(*prev_features)
        new_features = self.quant(self.conv2(self.relu2(self.quant(self.norm2(bottleneck_output)))))
        if self.drop_rate > 0:
...
class _DenseBlock(nn.Module):
    def __init__(self, num_layers, num_input_features, bn_size, growth_rate, drop_rate, quant, efficient=False):
...
class DenseNet(nn.Module):
    ...
    def __init__(self, growth_rate=12, block_config=(16, 16, 16), compression=0.5,
                 ...
                 forward_wl=-1, forward_fl=-1, backward_wl=-1, backward_fl=-1,
...
class DenseNetBC100LP:
    base = DenseNet
    args = list()
    kwargs = {"growth_rate":12, "block_config":(16, 16, 16),
              "small_inputs":True, "efficient":False}

class DenseNetBC190LP:
    base = DenseNet
    args = list()
    kwargs = {"growth_rate":40, "block_config":(31, 31, 31),
              "small_inputs":True, "efficient":False}
